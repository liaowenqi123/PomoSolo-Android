...
实测（2026-09-11，**MuMu 模拟器 Android 15 / x86_64 ↔ 桌面端真机互测**）：
在「设置 → 调试 → P2P 打洞测试」对桌面端账号发起两轮测试，**双向打洞全部成功**：

| 轮次 | 角色 | 结果 |
|------|------|------|
| A · 本机发起 | 安卓 offerer → 桌面端 answerer | ↑ **28.6 Mbps** · 2.0 MB / 560 ms |
| B · 对端发起（reverse） | 桌面端 offerer → 安卓 answerer | ↓ **77.3 Mbps** · 2.0 MB / 207 ms |

即 **Android ↔ 桌面端之间的 WebRTC DataChannel 直连真实可用**（走端到端直连，非服务器中转），
且信令/字段与桌面端实现完全互通（`tag` 路由、duplex 时序均验证通过）。
